...
Clarify Codex CLI token usage and identify which tool schemas were loaded in the current session.

### Changes Made

- Reviewed current session context and listed injected tool schemas.
- Clarified that external Codex CLI config/plugins are not visible without user-provided config.
- Provided focused token-optimization guidance for reducing schema and output overhead.

### Mistakes and fixes

- **Mistake:** Interpreted the request as project configuration at first.
- **Fix:** Refocused on Codex CLI context and tooling after clarification.
- **Prevention:** Confirm the target system earlier when “config” could refer to multiple layers.
...
- [ ] Review CLI config/plugin list once provided to pinpoint unused plugins/tool schemas.
